...
// void* MemoryManager::allocateVirtualMemory(enum AddressPoolType type, unsigned int size, unsigned int offset)
// {
//     unsigned int num_pages = (size + PAGE_SIZE - 1) / PAGE_SIZE;
//     int paddr = allocatePhysicalPages(type, num_pages);
...
//     printf("Allocated memory at virtual address: 0x%x (physical address: 0x%x)\n", vaddr, paddr);
//     return (void*)vaddr;
// }

// void MemoryManager::freeVirtualMemory(enum AddressPoolType type, void* addr, unsigned int offset)
// {
//     unsigned int vaddr = (unsigned int)addr;
//     unsigned int paddr = vaddr - offset;

//     int num_pages = 1; // 假设每次释放一页
//     releasePhysicalPages(type, paddr, num_pages);
//     printf("Freed memory at virtual address: 0x%x (physical address: 0x%x)\n", vaddr, paddr);
// }
